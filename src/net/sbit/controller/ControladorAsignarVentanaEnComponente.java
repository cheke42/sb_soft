...
public class ControladorAsignarVentanaEnComponente {
	...
	asignarVentanas();
	setearTabla();
	tableComponentes.getSelectionModel().selectedItemProperty()
		.addListener((observableValue, oldValue, newValue) -> {
		    // Check whether item is selected and set value of selected
		    // item to Label
		    if (tableComponentes.getSelectionModel().getSelectedItem() != null) {
			System.out.println(tableComponentes.getSelectionModel().getSelectedItem());
			tableComponentes.getItems().get(tableComponentes.getSelectionModel().getSelectedIndex());
			TablePosition pos = tableComponentes.getSelectionModel().getSelectedCells()
				.get(tableComponentes.getSelectionModel().getSelectedIndex());
			int row = pos.getRow();
			TableColumn col = pos.getTableColumn();

			CheckBox check = (CheckBox) col.getCellObservableValue(tableComponentes.getItems().get(row))
				.getValue();
	...
	tableComponentes.setItems(hashVentanas.get(
		TipoVentana.valueOf(comboVentana.getItems().get(comboVentana.getSelectionModel().getSelectedIndex()))));

    }

    // se fija si existe un elemento en la lista
